...
Para cada $i \in J_0^G$ (\refeq{J_0_level_superior}) debe de asignarse su multiplicador $\mu_i \geq 0$ correspondiente.
...
Para obtener las diferentes clases de puntos estacionarios debe de introducirse $\beta_j$, $\gamma_j$, en caso de ser este último valor de entrada tal que, los multiplicadores $\beta_j$  y $\gamma_j$ de forma tal que en los indices activos $J_1^v$ (\refeq{J_0_lambda_0_level_inferior}): 
\begin{itemize}
    %C estacionario
...
Primero debe tenerse un problema de optimización Binivel SLSF planteado como el \refeq{eq:DefBinivelOptimista}, el punto que se desea que sea estacionario ($z_{est}$), los índices activos descritos anteriormente 
