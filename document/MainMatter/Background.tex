...
%TODO: Como se dijo en la intro existe otra forma, en un problema de programación matemática  usando función del valor extremal sin embargo salvo en casos muy particulares esta no tiene una expresion explicita. La ventaja del kkt por MPEC es que es una forma explicita, pero se requiere algún tipo de condición de regularidad del conjunto como LICQ independencia  de v activa, que caso de no cumplirse poder existir puntos óptimos que no lo recoda el kkt. 
...
%TODO: incorporar indices y rango en que se mueven
\begin{equation}
\begin{aligned}
\min  &\quad  f(z)  \\
\text{s.t.} &\quad \begin{matrix} g_i(z)& \leq& 0, &i=1,\ldots,q, \\ h_k(z) &=& 0,&k=1\ldots,m, \\
...
%TODO: USar multiplicadores análogos a como esta en el binivel de miu lambda beta 
...
\subsection*{JuMP}
Esta biblioteca permite resolver problemas de programación matemática no lineal y discreta. O sea
$$
    \min F(z)$$
sujeto a:
\begin{align}
...
JuMP  permite gestionar una variedad de casos diferenciando como si las variables son continuas o discretas y  las restricciones  cuadráticas, lineales, de cotas o no lineales y, similarmente, si   la función objetivo es cuadrática, lineal o no lineal. También permite definir  restricciones de complementariedad  y ofrece métodos para resolver MPECs como ******.
  %TOD: Poner esto después de explicar las reformulaciones
Para m\'as detalles sobre lq biblioteca, ver \cite{JuMPPaper}.
...
Para el problema del nivel superior gestiona sus restricciones y la función objetivo de forma análoga. 
...
%TODO: MOdificar esto para decir que hace esto.
    ...
    %TODO: POner que es común a los dos 
