...
& \text{sujeto a } \\
& G(x) \leq 0, \\
& y \in \argmin_{y} \left\{ f(x, y) \mid g(x, y) \leq 0 \right\}
\end{aligned}
...
& \text{con } S(x) = \argmin_{y} \left\{ f(x, y) \mid g(x, y) \leq 0 \right\}
\end{aligned}
...
%% Métodos de Reformulación para Optimización Binivel
%\section{Métodos de Reformulación para Optimización Binivel 2}
%
...
%\label{subsec:BigM}
%
%El método Big-M (Fortuny-Amat y McCarl) es una técnica fundamental para reformular problemas MPEC en programas de enteros mixtos (MIP). 
%Para cada condición de complementariedad $v_j(x,y)\lambda_j = 0$ en \eqref{eq:KKT_Optimista}, se introduce una variable binaria $f_j \in \{0,1\}$ y cotas superiores $M_p$, $M_d$:
%
%\begin{align}
...
%\end{align}
%
%Esta reformulación linealiza las condiciones de complementariedad, permitiendo el uso de solvers MIP. La selección de $M_p$ y $M_d$ es crítica para evitar exclusión de soluciones o inestabilidad numérica \cite[Sección 4.1.3]{BilevelJump}.
%
%\subsection{Método SOS1}
%\label{subsec:SOS1}
%
...
%\end{equation}
%
%Esta restricción fuerza que al menos una variable en el par sea cero, preservando la no linealidad original sin necesidad de cotas. Es particularmente eficaz en problemas con estructura cónica \cite[Sección 4.1.1]{BilevelJump}.
%
%\subsection{Método ProductMode}
%\label{subsec:ProductMode}
%
%Este método mantiene las condiciones de complementariedad en su forma no lineal original, aplicable cuando se usan solvers NLP:
%
%\begin{equation}
%v_j(x,y) \cdot \lambda_j \leq t \label{eq:ProductMode_reg}
%\end{equation}
%
%donde $t > 0$ es un parámetro de regularización pequeño. Aunque no garantiza optimalidad global, es útil para obtener soluciones iniciales y manejar restricciones no lineales complejas \cite[Sección 4.1.4]{BilevelJump}.
%
